...
class ActivityUpdateForm extends Model
{
    ...
    public function rules()
    {
        return [//要求必须存在值
            ...
            /*在此注释上面两行是因为当修改已过期的活动时要求修改开始时间不早于当前时间，这是不合理的
            但是在创建时要求不早于当前时间时有意义的，因为活动创建时不可能是已过期的
             * */
    ...
    {
        /*由于是表单模型，所以在页面写入的数据只是在表单里，并未存入数据库，
        此动作用于完成页面数据向数据库存入数据
         * */
        $changename=false;
        ...
        //由于之前进行了一次validate操作，到此肯定是符合标准的，所以save(false)
        //不走rules验证规则，其次，这也是必要的，因为此时如果未改名，会违反唯一性
